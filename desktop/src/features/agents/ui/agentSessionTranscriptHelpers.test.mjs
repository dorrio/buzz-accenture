...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is an "Accenture Connect event", there is no event content to
  // surface, so userText is empty and the title falls through to
  // "Accenture Connect event".
  ...
  assert.equal(result.userTitle, "Accenture Connect event");
...
test("parsePromptText defaults the title to 'Accenture Connect event' when no kind is present", () => {
