...
export function HostedCommunitiesSettingsCard() {
    ...
    if (
      ...
      throw new Error(
        errorMessage(
          ...
          "Could not load the connected Accenture Connect identity.",
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run("Connecting Accenture Connect identity…", async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not connect the Accenture Connect identity.",
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the Accenture Connect identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected Accenture Connect identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Accenture Connect identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first."
            : errorMessage(
                bound.error,
                bound.correlation_id,
                "Could not connect this device's Accenture Connect identity.",
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            "That Accenture Connect address is already taken.",
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description="Accenture Connect works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone."
      />
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            ...
            Accenture Connect. You can use every other part of the app without
            signing in.
          </p>
          <Button
            className="mt-4"
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              <h3 className="font-medium">
                Link this account to your Accenture Connect identity
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                This Builderlab account isn&apos;t linked to an Accenture
                Connect identity yet. Connect this device&apos;s key to create
                and own communities under it — Accenture Connect signs a
                one-time challenge locally, so your private key never leaves
                Desktop.
              </p>
              <Button
                className="mt-4"
              ...
              >
                ...
                {action ?? "Connect Accenture Connect identity"}
              </Button>
            </div>
          ) : identityMismatch ? (
            <div className="rounded-xl border border-amber-500/50 bg-amber-500/5 p-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="font-medium">
                    This account is connected to a different Accenture Connect
                    identity
                  </h3>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Your Builderlab account owns communities under another
                    Accenture Connect key, so connecting them here would join a
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Accenture
                Connect identity connected
                {identity.npub ? (
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Unpair this Accenture Connect identity?
          </AlertDialogTitle>
          <AlertDialogDescription>
            Your Builderlab account will no longer be connected to this
            Accenture Connect key. You can reconnect any key later, but
            community actions stay unavailable until you do.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected Accenture Connect identity
